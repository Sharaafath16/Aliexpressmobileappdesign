...
import { ArrowLeft, Filter, ArrowUpDown, Zap } from "lucide-react";
...
export function SportsPage({ onBack, onProductClick }: SportsPageProps) {
  ...
  const filteredProducts = sortProducts(sportsProducts);

  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-white sticky top-0 z-50">
        <div className="p-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl">Sports & Outdoors</h1>
            </div>
          </div>
      ...
      <div className="px-4 pt-3">
        <PromoBanner
          ...
          subtitle="Get fit with amazing deals"
          buttonText="Shop Now"
          onButtonClick={() => {}}
        />
      </div>
      ...
      <div className="bg-white px-4 py-3 border-b mt-3">
        <p className="text-sm text-gray-600">
          {filteredProducts.length} sports products
        </p>
      </div>

      {/* Featured Categories */}
      <div className="px-4 py-3 bg-white mt-3">
        <h3 className="text-sm mb-3">Shop by Activity</h3>
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {["Fitness", "Running", "Cycling", "Yoga", "Swimming", "Outdoor"].map((cat) => (
            <button
              key={cat}
              className="px-4 py-2 bg-green-50 text-green-600 rounded-full text-sm whitespace-nowrap hover:bg-green-100 transition-colors"
            >
              {cat}
            </button>
          ))}
