...
import { ArrowLeft, Filter, ArrowUpDown, Heart } from "lucide-react";
...
export function BeautyPage({ onBack, onProductClick }: BeautyPageProps) {
  ...
  const filteredProducts = sortProducts(beautyProducts);

  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* Header */}
      <div className="bg-gradient-to-r from-pink-500 to-rose-500 text-white sticky top-0 z-50">
        <div className="p-4">
      ...
      <div className="px-4 pt-3">
        <PromoBanner
          imageUrl="https://images.unsplash.com/photo-1596462502278-27bfdc403348?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
      ...
      <div className="bg-white px-4 py-3 border-b mt-3">
        <p className="text-sm text-gray-600">
          {filteredProducts.length} beauty products
        </p>
      </div>
      ...
      <div className="px-4 py-3 bg-white mt-3">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {["Skincare", "Makeup", "Hair Care", "Fragrances", "Tools", "Sets"].map((cat) => (
            <button
              key={cat}
              className="px-4 py-2 bg-pink-50 text-pink-600 rounded-full text-sm whitespace-nowrap hover:bg-pink-100 transition-colors"
            >
              {cat}
            </button>
          ))}
