...
import { ArrowLeft, Filter, ArrowUpDown, Shirt } from "lucide-react";
...
export function FashionPage({ onBack, onProductClick }: FashionPageProps) {
  ...
  const filteredProducts = sortProducts(fashionProducts);

  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white sticky top-0 z-50">
        <div className="p-4">
      ...
      <div className="px-4 pt-3">
        <PromoBanner
          ...
          subtitle="Trending styles up to 70% OFF"
          buttonText="Shop Now"
          onButtonClick={() => {}}
        />
      </div>
      ...
      <div className="bg-white px-4 py-3 border-b mt-3">
        <p className="text-sm text-gray-600">
          {filteredProducts.length} fashion items
        </p>
      </div>

      {/* Featured Categories */}
      <div className="px-4 py-3 bg-white mt-3">
        <h3 className="text-sm mb-3">Shop by Style</h3>
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {["Men's Wear", "Women's Wear", "Kids", "Accessories", "Shoes", "Bags"].map((cat) => (
            <button
              key={cat}
              className="px-4 py-2 bg-purple-50 text-purple-600 rounded-full text-sm whitespace-nowrap hover:bg-purple-100 transition-colors"
            >
              {cat}
            </button>
          ))}
